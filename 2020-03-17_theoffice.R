...
## Reproducing a graph from @drob's analysis
...
## Reproducing the original plots from the pudding's project at https://pudding.cool/2017/08/the-office/
office_lines <- schrute::theoffice

# Lines spoken during the entire series by characters who appeared in at least six episodes

office_lines %>%
  mutate(id = paste(season, episode, sep = "_")) %>% ## unique id for each episode
 # group_by(id) %>%
